...
import com.hanna2100.cleannote.framework.presentation.common.changeColor
...
class NoteListAdapter(
    ...
    fun submitList(list: List<Note>) {
        ...
        printLogD(this.javaClass, "size: ${list.size}")
            ...
            selectedNotes.observe(lifecycleOwner, Observer { notes ->
                ...
                if(notes != null){
                    if(notes.contains(note)){
                        changeColor(newColor = COLOR_GREY)
                    }
                    else{
                        changeColor(newColor = COLOR_PRIMARY)
                    }
                }else{
                    changeColor(newColor = COLOR_PRIMARY)
                }
